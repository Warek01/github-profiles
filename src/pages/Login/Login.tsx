...
const Login: React.FC<LoginProps> = ({ errorMessage, onLogin, registeredUsers, removeRegisteredUser }) => {
  ...
  const handleInputFormKeydown = React.useCallback(
  ...
  const errorElement: JSX.Element = (
    <Collapse in={errorShown}>
      <Alert
        severity="error"
        action={
          <IconButton size="small" onClick={() => setErrorShown(false)}>
  ...
  const authSectionElement: JSX.Element = oauthInputShown ? (
    <Input
      type="password"
      autoComplete="off"
    ...
    <Button onClick={() => setOauthInputShown(true)}>oauth key</Button>
  )

  React.useEffect(() => {
    if (oauthInputShown) oauthInputRef.current!.focus()
    else setOauthToken('')
  }, [oauthInputShown])
      ...
      <Grid container justifyContent="center" sx={{ height: '40vh', margin: '4vh 0' }}>
        <Card sx={{ ...cardWidth, padding: '30px 0' }} onKeyDown={handleInputFormKeydown}>
          <Grid container spacing={1} justifyContent="center" alignItems="center" flexDirection="column">
            ...
            <Grid item>{errorElement}</Grid>
            <Grid item>{authSectionElement}</Grid>
            <Grid item>

...
const LangInfo: React.FC = () => {
  const { showSnackbarThenHide } = React.useContext(SnackbarContext)
  ...
  const getLangInfo = React.useCallback(async (): Promise<void> => {
    const langMap = await analyzeReposLanguages(userProfile!.repos)
    showSnackbarThenHide(`Loaded in ${langMap.elapsedMs} ms`)
  ...
  const handleAccordionExpansion = React.useCallback(async (): Promise<void> => {
    if (!langList.length) await getLangInfo()
  ...
  return (
    <Accordion
      ...
      <AccordionSummary expandIcon={<ExpandMore />} onClick={handleAccordionExpansion}>
        <Typography sx={{ color: 'primary.main' }}>Languages</Typography>
      </AccordionSummary>
